...
def parse_line_to_message(line):
    ...
    return {'name': name, 'timestamp': timestamp, 'freq': freq, 'access': access, 'lead_out': lead_out,
            'confidence': confidence, 'strength': strength, 'length': length, 'error': error, 'msgtype': msgtype}


def print_message(m):
    print("RAW:", m['name'], m['freq'], "%06d" % m['timestamp'], 'A:%s' % m[access], 'L:%s' % m[lead_out], '%03d%%' % m[
        'confidence'], "%03d" % m['length'])
